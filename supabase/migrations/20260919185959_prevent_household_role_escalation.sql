-- Bootstrap must preserve an existing membership role. In particular, a
-- non-owner member reopening the app must not be promoted to owner.
...
-- An optional database event trigger helper was installed outside the app
-- migrations. It is not an API endpoint and must not be callable by clients.
do $$
begin
